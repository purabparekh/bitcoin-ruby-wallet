...
def validate_sendtoaddress_inputs ( data )
  ...
  begin
    ...
    if transaction == nil
      response["err_msg"] = "Either invalid, non-wallet or already spent Transaction ID, or incorrect vout index"
      return response
    end

    if is_multisig_transaction(transaction[:trans_id],transaction[:vout_index])
      response["err_msg"] = "Transaction ID provided is a multisig transaction. Please use `redeemtoaddress` command for this."
      return response
    end

    if btc_to_satoshi(transaction[:value]) < (data[2] + TRANSACTION_FEE)
      response["err_msg"] = "Amount to transfer must be less than the transaction amount"
      return response
    end
...
def validate_sendtomultisig_inputs ( data )
  ...
  begin
    ...
    if data[3] > payee_addresses.length
      response["err_msg"] = "Minimum signatures required must be less than or equal to the number of payee addresses provided"
      return response
    end
    ...
    payee_addresses.each { |address|
      ...
      if !is_valid_address address
        response["err_msg"] = "Either invalid or non-wallet payee address: " + address
        return response
      end
    }
...
def validate_redeemtoaddress_inputs ( data )
  ...
  begin
    ...
    if !is_multisig_transaction(transaction[:trans_id],transaction[:vout_index])
      response["err_msg"] = "Transaction ID provided is not multisig transaction. Please use `sendtoaddress` command for this."
      return response
    end
